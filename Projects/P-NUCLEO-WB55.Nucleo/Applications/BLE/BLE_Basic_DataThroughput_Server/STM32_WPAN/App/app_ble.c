...
/* USER CODE BEGIN PTD */
uint8_t TimerMeasurement_Id;   //[JT]
/* USER CODE END PTD */
...
uint8_t ad_data[21] = {
    ...
    8, AD_TYPE_COMPLETE_LOCAL_NAME, 'D', 'T', 'S', '-', 'S', 'R', 'V',  /* Complete name */

    8, AD_TYPE_MANUFACTURER_SPECIFIC_DATA, 0x01, 0x83, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */,
};
...
/* USER CODE BEGIN APP_BLE_Init_2 */
   //[JT]
   HW_TS_Create(CFG_TIM_PROC_ID_ISR, &TimerMeasurement_Id, hw_ts_Repeated, payloadTimerCallback);
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification( void *pckt )
{
  ...
  hci_le_phy_update_complete_event_rp0 *evt_le_phy_update_complete;  //[JT]
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (meta_evt->subevent)
      {
        case HCI_LE_CONNECTION_UPDATE_COMPLETE_SUBEVT_CODE:
          ...
          UTIL_SEQ_SetEvt(1 << CFG_MY_IDLEEVT_CONN_UPDATE_COMPLETE);  //[JT]
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      /* USER CODE BEGIN BLUE_EVT */
        case ACI_GATT_TX_POOL_AVAILABLE_VSEVT_CODE:
          //APP_DBG_MSG("ACI_GATT_TX_POOL_AVAILABLE_VSEVT_CODE \n");  
          Enable_Flow();  //[JT] - Application can try sending data again
          break;
...
//[JT]
static void payloadTimerCallback(void)   
{
...
static void LinkConfiguration(void)
{
  ...
  {  
    APP_DBG_MSG("HCI_LE_SET_DATA_LENGTH Update, Successfully \n");    
  }
  else
  {
    APP_DBG_MSG("HCI_LE_SET_DATA_LENGTH Update, Error \n");   
  }  

  /* Set ATT MTU size */  
  APP_DBG_MSG("Request ATT MTU size exchange\n");
  
  status = aci_gatt_exchange_config(BleApplicationContext.BleApplicationContext_legacy.connectionHandle);
  if( status == BLE_STATUS_SUCCESS )
  {  
    APP_DBG_MSG("ACI_GATT_EXCHANGE_CONFIG Update, Successfully \n");    
  }
  else
  {
    APP_DBG_MSG("ACI_GATT_EXCHANGE_CONFIG Update, Error \n");    
  }  
...
//  uint16_t interval_max = CONN_P(48.75);    
  ...
  if( status == BLE_STATUS_SUCCESS )
  {
    APP_DBG_MSG("ACI_L2CAP_CONNECTION_PARAMETER_UPDATE_REQ, Successfully \n\r");
  }
  else
  {
    APP_DBG_MSG("ACI_L2CAP_CONNECTION_PARAMETER_UPDATE_REQ, Error \n\r");
  }
  
  UTIL_SEQ_WaitEvt(1 << CFG_MY_IDLEEVT_CONN_UPDATE_COMPLETE);  /* Wait for Connection Update complete event to be recieved before proceeding */
  ...
  if (status == BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("HCI_LE_READ_PHY, Succesffuly \n");    
    APP_DBG_MSG("Current PHY: TX= %d, RX= %d \n", TX_PHY, RX_PHY);    
  }
  else 
  {
...
//[JT] - Toggles between the two PHYs
void PHY_Configuration(void)
{
  tBleStatus status;
  uint8_t TX_PHY, RX_PHY;
  
  /* Set PHY config */    
  APP_DBG_MSG("Set PHY Config \n");
  ...
  if (status == BLE_STATUS_SUCCESS)
  {
    ...
    if ((TX_PHY == TX_2M) && (RX_PHY == RX_2M))
    {
      ...
      status = hci_le_set_phy(BleApplicationContext.BleApplicationContext_legacy.connectionHandle,ALL_PHYS_PREFERENCE,TX_1M,RX_1M,0);
    }
    else
    {
      APP_DBG_MSG("**TX= %d, **RX= %d \n", TX_2M_PREFERRED, RX_2M_PREFERRED);
      status = hci_le_set_phy(BleApplicationContext.BleApplicationContext_legacy.connectionHandle,ALL_PHYS_PREFERENCE,TX_2M_PREFERRED,RX_2M_PREFERRED,0);
    } 
    if( status == BLE_STATUS_SUCCESS )
    {
      APP_DBG_MSG("HCI_LE_SET_PHY Update, Successfully \n\r");
    }
    else
    {
      APP_DBG_MSG("HCI_LE_SET_PHY Update, Failed , status: 0x%X\n\r", status);
    }  
  }
  else 
  {
